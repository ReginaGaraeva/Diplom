			...
			while (!sr.EndOfStream)
			{
				...
				if (buf.IndexOf("PVT") == 0)
				{
					...
					while (buf != "/")
					{
						...
						{
							P = Convert.ToDouble(values[0]),
							ZData = new ZInterpolationData
							{
								Ro = Convert.ToDouble(values[1]),
								V = Convert.ToDouble(values[2]),
								C = Convert.ToDouble(values[3]),
								G_gas = Convert.ToDouble(values[4]),
								Ro_gas = Convert.ToDouble(values[5]),
								Ro_cond = Convert.ToDouble(values[6])
							}
						});
						buf = sr.ReadLine();
					}
