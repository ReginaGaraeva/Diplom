...
public class DBService
{
	...
	public void LoadWellsMeasurementsToDB(List<WellData> wells)
	{
		...
		for (int i = b; i < e; i++)
		{
			...
			dbEntities.wells_measurements.Add(new wells_measurements
			{
				well_id = id,
				cond_output = (float)wells[i].G_condensat,
				gas_output = (float)wells[i].G_gas,
				inlet_P = (float)wells[i].P_shl,
				inlet_T = (float)wells[i].T_shl,
				measure_date = wells[i].Date.Date,
				wellhead_P = (float)wells[i].P_ust,
				wellhead_T = (float)wells[i].T_ust
			});
		}
		dbEntities.SaveChanges();
	}
