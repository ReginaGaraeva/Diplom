...
namespace FieldDataAnalyzer
{
	class EvaluatorUnknownG
	{
		...
		public double beta_kr = 0.6, Gkr = 15, Pkr = 35000000;
		private FieldDataAnalyzerDBEntities db = new FieldDataAnalyzerDBEntities();

		public EvaluatorUnknownG(Graph graph, FieldDescription fieldData, ToolStripProgressBar _toolStripProgressBar)
		{
			_graph = graph;
		...
		private void CalcPipe(Pipe pipe)
		{
			...
			for (int m = 0; m < 5; m++)
			{
				...
				pipe.EndNode.T_[m] += (pipe.StartNode.T_[m] - dt);
				...
				if (pipe.StartNode.P_[m] > dp) pipe.EndNode.P_[m] += pipe.StartNode.P_[m] - K_p*dp;
		...
		private void CalcNode(Node node)
		{
			if (node.wells.Count != 0)
			{
				...
				for (int i = 0; i < node.wells.Count(); i++) //выполняем расчет G для всех скважин
				{
					...
					if ((node.wells[i].G_condensat + node.wells[i].G_gas) == 0) continue;
					if (firstWell == null) //если скважина первая, то считаем для нее давление
					{
						firstWell = node.wells[i];
						node.wells[i].P[0] = beta_kr * node.wells[i].P_ust;
						node.wells[i].G[0] = Math.Sqrt(node.wells[i].a[2])
							* (node.wells[i].G_condensat + node.wells[i].G_gas);
						for (int m = 1; m < 5; m++)
						{
							node.wells[i].P[m] = (m * 0.25 * (1 - beta_kr) + beta_kr) * node.wells[i].P_ust;
				...
				for (int i = 0; i < 5; i++)
				{
					...
					for (int j = 0; j < node.wells.Count(); j++)
						TG += node.wells[j].T_shl*node.wells[j].G[i];
